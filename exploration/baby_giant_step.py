...
def expand_to_dangling_leaves(tree: Tree, start_config: Configuration, leaves: Set[str]) -> Traversal:
    ...
    for d in range(1, depth+1):
        ...
        for j, leaf in enumerate(current_frontier):
            ...
            from_remainder = min(remainder, len(leaf_detected))
            next_config[leaf] += from_remainder
            remainder -= from_remainder
...
def baby_giant_step(tree: Tree,
    ...
    current_depth = max(tree.depth(leaf) for leaf in leaves)
    for t in range(2*picaboo_counter):
        ...
        for leaf in leaves:
            ...
            if res:
                ...
                parallel_picaboos.append(picaboo_session)
        zipped_picaboos = list(zip_longest(*parallel_picaboos, fillvalue=Counter()))
